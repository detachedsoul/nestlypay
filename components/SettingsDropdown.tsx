...
const SettingsDropdown: React.FC<dropdownProps> = ({
	isOpen,
}: dropdownProps) => {
	...
	return (
		<div
			className={`bg-white absolute space-y-2 w-max right-0 top-[calc(100%+1.5rem)] z-[9999] py-4 rounded-lg transition-transform ease-in-out duration-500 shadow-2xl ${isOpen ? "translate-y-0" : "-translate-y-[200%]"}`}
		>
			<Link
				className={cn(
