		...
		<>
			<header className="sticky top-0 z-50">
				<div className="flex items-center gap-4 justify-between py-4 px-6 bg-white/100 border-b border-[rgba(241,_241,_241,_1)] lg:hidden">
					<button
						className="bg-[#0c46d3]/[0.05] text-brand-blue rounded-lg p-1.5"
						type="button"
					...
					<div className="flex items-center gap-4 relative">
						...
						>
							<Image
								className="size-12 oject-center object-cover aspect-square rounded-full"
								src={authInfo?.imageUrl ?? UserImage}
								alt={authInfo?.name ?? ""}
								width={48}
								height={48}
							/>
				...
				<div className="bg-white/100 py-4 px-8 hidden lg:flex items-center justify-between gap-4 sticky z-[1024] top-0">
					<form className="w-1/2">
						<label
							className={cn(
								"input",
								"w-full shrink-0 flex items-center py-0",
							)}
							htmlFor="search"
						>
							<input
								className="py-3 focus:outline-none w-full"
								type="search"
								placeholder="Search"
								name="search"
								id="search"
							...
							<SearchIcon
								className="text-[rgba(151,_151,_151,_1)]"
								size={18}
							/>
						</label>
					</form>
